...
Лужа под шторой — не декорация, а след: в ванной забыли закрыть кран. Поэтому вода и лужа включаются **одним вызовом** `StoryManager.SetPuddles(true)` (глава с чипом, там же открываются шторы), а выключает их игрок — перекрыв кран.
...
- [Puddle](Assets/Scripts/Puddle.cs) — на самой луже. `Dry()` сжимает её до 5% и выцветает за 3.5 сек, в конце выключает объект. `OnEnable` возвращает исходный масштаб и альфу: второе прохождение должно начинаться с целой лужи.
- **Кран и лужи не знают друг о друге по ссылкам, и это единственный рабочий способ.** Лужа — переименованный инстанс `QuadObj` внутри `livingroom.prefab`, кран — внутри вложенного `watercloset.prefab`; сослаться из одного на другое в инспекторе нельзя. Оба компонента ведут статические списки того, что сейчас активно (регистрация по `OnEnable`/`OnDisable`), и `StopAll` перекрывает всё, что течёт, а `Puddle.DryAll` сушит всё, что лежит. Луж и кранов при этом может быть сколько угодно — сейчас их по две (в лабиринте две копии гостиной), и добавление третьей не требует ни строчки кода.
...
- **`PuddleZone.OnDisable` отпускает игрока из мокрых шагов.** `Footsteps._puddleCount` накопительный, а `OnTriggerExit` на выключение объекта приходит не всегда — и лужа, высохшая (или выключенная вместе с лабиринтом в `HUD.TeleportBack`) прямо под ногами, оставляла бы игрока «мокрым» до конца игры.
...
- [Openable](Assets/Scripts/Openable.cs) — шкаф/полка с галлюцинаторной подменой содержимого. Клик мышью открывает и закрывает дверцу, щелчок пальцем (`Q` → `SnapAllVisible`) меняет содержимое по кругу у всего, что сейчас в кадре. **Один щелчок = одна смена содержимого, независимо от того, открыт предмет или закрыт**: у закрытого играется хлопок дверцей и подмена идёт через `swapAt`, у открытого дверца закрывается и подмена идёт в конце анимации закрытия (`CloseDuration` = длина того клипа, который реально играет `Close`). Раньше на открытом шкафу уходило два щелчка — первый только закрывал, второй менял. Пока идёт анимация, `_isChanging` блокирует и повторный щелчок, и клик мышью. Флаг `ReactsToSnap` выключает реакцию на щелчок совсем — он снят у обеих штор (`openable_curtain_livingroom`, `curtain_kitchen`): содержимое за ними больше не перещёлкивается (`states` пуст), а дёргаться на каждый щелчок они продолжали. `PlayerInside` проверяется прямо перед подменой: если игрок за время анимации зашёл внутрь комнаты-состояния, прятать её нельзя — он провалится в пустоту.
