...
- **Проверяй, что квад вообще внутри комнаты.** Часы (`QuadObj_4`, спрайт `clock.png` — это наручные часы, не настенные) лежали в `livingroom.prefab` на локальном x = 3.135 при восточной стене комнаты около x ≈ 1.37, то есть **за стеной**, лицом обратно в комнату. Выглядело это как «часов нет вообще, но по пустому месту можно ударить и слышен звон»: спрайт односторонний и закрыт стеной, а коллайдер `BreakableProp` живой. Сейчас они на восточной стене — локальные `(1.2, 1.9, -3)` и масштаб `0.4` (0.21 × 0.63 м, читается как настенные часы; при масштабе 1 арт даёт полутораметровые наручные часы). Двигать — этими двумя строчками в инстансе `QuadObj_4`.
