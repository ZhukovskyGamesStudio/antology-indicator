...
[Assets/Scripts/MadnessVignette.cs](Assets/Scripts/MadnessVignette.cs) на `UICanvas/OtherUi/Vignette`. Общая альфа ведётся по `_alphaCurve` от процента безумия: до 35% нуль, 0.45 на 70%, 1 на 100%. Лежит внутри `OtherUi`, поэтому автоматически прячется в паузе и на экранах победы/проигрыша.
...
|---|---|---|---|---|
| подложка | `OtherUi/VignetteBg` (sibling 0) | `vignette_bg_1..2` | 0.6 | 0.23 с, фаза +0.07 |
| рамка | `OtherUi/Vignette` (sibling 1) | `vignette_front_1..3` | 1.0 | 0.13 с |

Арт лежит в `Assets/ToSort/vignette/`. Что поверх чего — решает порядок объектов в иерархии, а не порядок в списке слоёв: подложка обязана стоять раньше рамки. Фазы разведены специально — если кадры перещёлкиваются синхронно, это читается как подёргивание всей картинки, а не как живая линия. Потолок подложки занижен до 0.6, потому что края и так мягко темнит `Vignette` из `MoodDirector`. Кадры не листаются, пока альфа в нуле: смена спрайта помечает канвас грязным даже у полностью прозрачной картинки.

Старый рисунок `Assets/ToSort/ui_eye/vignette.png` остался только на экране титров (`WinPanel/Vignette`, статичная альфа 0.8) — там своя логика и анимация ему не нужна.
