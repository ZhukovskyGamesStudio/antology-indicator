...
- **Целевые платформы (CI):** только StandaloneWindows64 — собирается GitHub Actions и публикуется через butler на itch.io (`grafenters.itch.io/zhukovsky-games`). Сборки StandaloneOSX и WebGL из пайплайна убраны (см. CI/CD).
...
- «Раньше тут лежало другое... Всё меняется как по щелчку пальцев» — это **подсказка про механику щелчка**, поэтому она висит только на [OpenableShelf.prefab](Assets/Prefabs/OpenableShelf.prefab) (`InteractiveObj.OnClick` → `ReactOnce`), то есть на полках `MirrorRoom` — единственных шкафах в игре, у которых `Openable.states` не пуст и содержимое реально перещёлкивается. Такой же `ReactOnce` какое-то время висел на шкафу в прихожей ([Hallway.prefab](Assets/Prefabs/Rooms/Hallway.prefab) → `wardrobe_final`), у которого `states: []` — а дедуп идёт по тексту реплики, так что подсказка сгорала на пустом шкафу и до настоящих полок не доживала. Убрано. **Вешаешь реплику-подсказку — вешай на тот объект, на котором механика есть.**
...
### Материалы поверхностей (smoothness / карты нормалей)

Отражений в сцене нет (`reflectionIntensity = 0`, проб нет), поэтому «блеск» — это только прямые спекуляр-блики от ламп; главный усилитель — `HeldItemLight` в 25 см от предмета в руках. Раньше почти всё сидело на дефолтном smoothness 0.5 из FBX/URP и бликовало как пластик.

**Встроенные в FBX материалы (вся мебель, 19 книг, 13 папок, сантехника) правит [Assets/Editor/MaterialImportTuner.cs](Assets/Editor/MaterialImportTuner.cs)** — таблица правил «текстура/путь → smoothness, metallic, спекуляр, нормал» применяется на каждом импорте модели. В инспекторе такие материалы не редактируются, крутить значения = править правило в этом файле. Правила поменял — **подними `Version` и запусти `Tools → Материалы → Переимпортировать модели`**, иначе Unity не переимпортирует.
- **Грабля пайплайна: `OnPostprocessMaterial` для FBX НЕ вызывается** (пайплайн MaterialDescription, дефолт с 2019.3) — тюнер висит на `OnPostprocessModel` и обходит материалы рендереров.
- Ключевые значения: бумага/книги/папки — smoothness 0.05 и **спекуляр выключен совсем** (`_SPECULARHIGHLIGHTS_OFF`; блик в руках убивался только так); корпусная мебель 0.18; лак (стол/табуретки/стул) 0.32; двери-эмаль 0.28; обои/стены/потолок 0.03 + спекуляр off; керамика 0.5–0.55; краны metallic 0.35 + smoothness 0.55 (**полный металлик нельзя** — без отражений металл станет чёрным); техника-эмаль 0.3–0.4; стёкла 0.6–0.9; ткань дивана 0.05 + спекуляр off.
- Обычные `.mat`-файлы проекта настроены напрямую по тем же типам (обои, потолок, полы, лампы-абажуры 0.12+specOff, холодильник/микроволновка, окна). Кастомный `LitSprite` (все квады-картинки) — чистый ламберт **без спекуляра**, квады в тюнинге не участвуют.
- **Служебная геометрия** (TriggerToClose полок, PortalCenter/TeleportBack) и подмеши без материала сидели на нередактируемом пакетном `Lit.mat` (0.5) — заменены на [Assets/Materials/DefaultMatte.mat](Assets/Materials/DefaultMatte.mat) (тот же белый, smoothness 0.15).

**Карты нормалей ПРОБОВАЛИ И ОТКАТИЛИ — не возвращать без нового решения.** Генерация из albedo («Create from Grayscale» по уменьшенным копиям текстур) давала рельеф швам и филёнкам, но по вердикту плейлука «во многих местах стало хуже»: рисованный стиль плоских заливок с контурами превращался в «шагрень» и штукатурку. Вычищено полностью: папки `Models/Final/Normals/` больше нет, у тюнера нормалей нет (Version 5), у прямых .mat слоты `_BumpMap` пусты. Если рельеф когда-нибудь понадобится — это должны быть нарисованные художником карты, не автогенерация.

`EyeSpawner.DarknessThreshold` эти правки не трогают: менялся только спекуляр, не диффузная яркость.
...
Декоративные картинки-квады, которые игрок с молотком может разбить (ЛКМ): чайник, солонка и кружка на кухне ([kitchen.prefab](Assets/Prefabs/Rooms/kitchen.prefab)), телевизор, часы и ваза в гостиной ([livingroom.prefab](Assets/Prefabs/Rooms/livingroom.prefab)), два телевизора в [MirrorRoom.prefab](Assets/Prefabs/Rooms/MirrorRoom.prefab) (у стены и в полке `OpenableShelf_3`). Поломка = подмена спрайта на `*_broken` + короткая пружинка масштабом (`DOPunchScale`) + стук молотка + звон стекла. Молоток после третьей главы не отбирается (`SetHammer(false)` в сценарии нет), так что бить можно и в обычной квартире в главе с чипом.

- **Ломаемое живёт в префабе комнаты, а не в сцене — иначе в лабиринте его нет.** Ваза сначала стояла отдельным инстансом `QuadObj` **в корне `GameScene`**: в обычной квартире она была, а в лабиринте (это другое место в мире, копии той же гостиной) её не было вовсе, и после возврата из лабиринта бить её было нечем — сцену игрок видел ту же, а объект остался единственным. Теперь это `Vase` внутри [livingroom.prefab](Assets/Prefabs/Rooms/livingroom.prefab), то есть он приезжает во все три инстанса гостиной (один в `NormalRooms`, два в `LabirintRooms`). **Добавляешь новый ломаемый предмет — клади его в префаб комнаты.**
- **Проверяй, что квад вообще внутри комнаты.** Часы (`Clock`, бывший `QuadObj_4`; спрайт `clock.png` — это наручные часы, не настенные) лежали в `livingroom.prefab` на локальном x = 3.135 при восточной стене комнаты около x ≈ 1.37, то есть **за стеной**, лицом обратно в комнату. Выглядело это как «часов нет вообще, но по пустому месту можно ударить и слышен звон»: спрайт односторонний и закрыт стеной, а коллайдер `BreakableProp` живой. Сейчас они на восточной стене — локальные `(1.31, 2.09, -2.306)` и масштаб `0.71396` (0.38 × 1.13 м); при масштабе 1 арт даёт полутораметровые наручные часы, поэтому масштаб тут не декоративный, а обязательный. **В обеих гостиных лабиринта `m_LocalPosition.x` перебит per-instance** (1.284 и 1.289 в [LabirintRooms.prefab](Assets/Prefabs/Rooms/LabirintRooms.prefab)) — стены там стоят чуть иначе; правка x в базовом префабе туда не доедет, y/z и масштаб доедут.
...
- Перенос строк иконку не ломает: она встаёт после последнего символа, то есть на последней строке, а зарезервированные справа 86 px всегда при ней (плашка = 15 + самая длинная строка + 86).
...
- **Английский длиннее русского, и «обнимающие» плашки от этого вылезали за экран.** У субтитров, задач и финального заголовка плашка (`HorizontalLayoutGroup` + `ContentSizeFitter`) тянулась ровно по длине строки, и предела у неё не было: русские реплики помещались, а английские обрезались с обеих сторон (плашка субтитров отцентрована). Худший случай — «There was something else here before…» просилась на 1948 px при экране 1920. Лечится [TextWidthLimit](Assets/Scripts/Localization/TextWidthLimit.cs): sink после каждой смены текста навязывает подписи предельную ширину через `LayoutElement`, и длинная строка переносится вместо роста плашки. Пределы — поля `maxTextWidth` в инспекторе (`TalkUI` 1500, `TasksUI` и `WinPanel` 1400 при канвасе 1920).
  - Ширина берётся **после переноса** (`GetPreferredValues(text, maxWidth, 0)`), а не равной пределу — иначе плашка всегда раздувалась бы до максимума, даже когда вторая строка короткая. Строка короче предела → `preferredWidth = -1`, то есть поведение ровно как раньше.
  - `LayoutRebuilder.ForceRebuildLayoutImmediate` сразу после — обязателен: следом идёт `ForceMeshUpdate`, а он считает переносы по ТЕКУЩЕЙ ширине, и без пересборки первый кадр реплики верстался бы по ширине предыдущей.
  - **Добавляешь новый sink с обнимающей плашкой — подключи к нему `TextWidthLimit`**, иначе на английском он однажды уедет за экран.
- **Подписи фиксированного размера (кнопки) переведены на автоподбор кегля.** «Выйти в меню» → «Exit to Main Menu» не влезало в кнопки паузы, экрана смерти и финала (503 px при рамке 461). Там включён `enableAutoSizing` с выключенным переносом: кнопка всегда одна строка, кегль ужимается под самую длинную локализацию. Подсказка про книги в меню (`BooksHint`) — автоподбор с переносом, там это предложение, а не кнопка.
...
**Геометрия сцены почти ничего не стоит — кадр съедают тени и пост-обработка.** Разбор кадра фрейм-дебаггером (обычная квартира, 1920×1080): из 148 событий **60 (41%) — карты теней точечных ламп**, 16 — Bloom, 18 — прозрачные объекты, 12 — DepthNormals prepass + SSAO, 5 — DoF Bokeh, 3 — SMAA, а на всю опаковую геометрию квартиры уходит **15 событий**. То есть оптимизировать меши и материалы смысла нет, всё решают настройки света и постпроцесса.

**Стоимость по слагаемым** (грубый A/B в редакторе, цифры плавают): база ≈ 18.6 мс, тени ≈ +5.8 мс, пост-обработка ≈ +4.0 мс.

**Тени — главный резерв, но трогать их нельзя без изменения картинки.** Шесть люстр дают по 6 граней кубической карты каждая: 36 карт + спот настольной лампы = 37. В лабиринте активных теневых ламп 18 → **93 карты**, они не влезают в атлас 4096, и URP молча режет разрешение (в консоли `Reduced additional punctual light shadows resolution`) — то есть платим за максимум, а получаем мыло. Сокращать можно только ценой видимой разницы.

**Что уже сделано (и почему это безопасно).** Обе правки проверены **попиксельным сравнением кадров**: рендер в RenderTexture из нескольких ракурсов до и после, разница сравнивается с шумовым порогом (плёночное зерно рандомит кадр, поэтому даже два одинаковых рендера дают maxΔ=6, meanΔ≈1.4 — всё, что укладывается в этот порог, картинку не меняет).
- **Opaque Texture выключен** в `PC_RPAsset` / `Mobile_RPAsset`: `_CameraOpaqueTexture` не читает ни один шейдер проекта, а URP каждый кадр делал полноэкранную копию (проход `CopyColor`). Диффы: maxΔ=6 — на уровне шума. **Понадобится эффект вроде преломления стекла — флаг придётся вернуть.**
- **[HeldItemCameraToggle](Assets/Scripts/HeldItemCameraToggle.cs)** на `HeldItemCamera`: оверлей-камера рисует единственный слой `HeldItem`, который заполняется только пока предмет поднят, а работала она всегда — DepthNormals prepass + **4 прохода SSAO** + опак + блит ради пустого кадра. Гейт по статическому `FirstPersonController.isHolding` (он выставляется в `Pickable.TogglePick` ровно перед переносом объекта на слой, так что совпадает со слоем кадр в кадр). Итого 148 → **141 событие** в простое.

**Что проверено и сознательно НЕ сделано** (меняет картинку):
- **SSAO у предмета в руках** — казалось бы, тоже впустую, но замер показал maxΔ=86 при пороге 6: затенение на предмете реально видно. Чтобы убрать, понадобился бы отдельный Renderer-ассет без SSAO для этой камеры — и картинка бы изменилась.
- SSAO стоит на максимуме (`Downsample = False`, Samples и Blur = High) и тянет за собой отдельный DepthNormals prepass; понижение качества заметно.
- `MotionBlur` (0.365) и `FilmGrain` (0.14) — живые эффекты, не наследство; выключение меняет кадр.
- Bloom (6 итераций + HQ-фильтрация), DoF Bokeh, SMAA High — всё видимое.
- Уменьшение Shadow Distance (сейчас 50 м) даёт выигрыш ровно в той мере, в какой начинает срезать тени, то есть бесплатным не бывает.

**Что можно сделать безопасно, но руками:** запечь **Occlusion Culling** — сейчас его нет вообще (`umbraDataSize = 0`, Static-флаг стоит у 4 рендереров из 1057), а квартира состоит из комнат со стенами, и в лабиринте разом активны 480 рендереров против 250. Требует аккуратной расстановки Static только на неподвижной геометрии (не на дверцах, шторах и `Pickable`) и проверки, что лабиринт, выключенный на момент запекания, не начнёт исчезать. **Но сначала прочитай следующий абзац** — выигрыш от культинга здесь под большим вопросом.

**GPU Occlusion Culling (без запекания) пробовали — стало ВДВОЕ ХУЖЕ, не включать.** В Unity 6 есть `GPU Resident Drawer` (`Instanced Drawing`) с галкой `GPU Occlusion Culling` в URP-ассете: культинг по Hi-Z из глубины предыдущего кадра, запекать ничего не надо. Требует `BatchRendererGroup Variants = Keep All` в Graphics settings, иначе Unity ругается и молча не включается. Замер при одном и том же виде:

| | выключено | включено |
|---|---|---|
| треугольников | 20 712 | 10 152 (−51%) |
| draw calls | 237 | 267 (+30) |
| теневых кастеров | 112 | **112 (не изменилось)** |
| время кадра | 29–34 мс | 61–69 мс |

Культинг честно отсекает половину геометрии — и это ничего не даёт, потому что **сцена не упирается в геометрию**: во всём кадре 20 тысяч треугольников и 15 опаковых событий. Зато `GPU Resident Drawer` добавляет свои диспатчи (+30 draw calls) и переводит рендер на BRG-путь, которым **не может воспользоваться большая часть сцены**: 241 `SpriteRenderer` не поддерживается вовсе, 7 рендереров исключены из-за `MaterialPropertyBlock`, а кастомные шейдеры (`LitSprite`, `WorldTiledFloor`, `DustMote`, `LampHaze`) не объявляют DOTS-инстансинг. В итоге платим за два пути рендера одновременно. И главное: **на тени культинг не влияет** (112 кастеров в обоих случаях), а тени — это 41% кадра.

**Мёртвые настройки:** включены Main Light Shadows и 4 каскада, хотя **directional light в сцене нет ни одного**; `m_AdditionalLightsPerObjectLimit = 4` в Forward+ не применяется. На скорость не влияет, но путает при настройке.

**Скриптовая часть чистая:** в per-frame коде нет `GetComponent`/`Find`, физика на дефолтном шаге (212 кинематик-тел спят), `Canvas.PrepareBatches` ≈ 33 мкс. Профилировать CPU смысла нет — узкое место в GPU.
...
- **`steam_appid.txt` в корне проекта — только для редактора.** Steam читает его при запуске не из клиента; в билд файл не попадает (CI собирает в `build/`), Steam-клиент подставляет ID сам. Сейчас там **4823510** — настоящий App ID игры (в Steam она называется «Broadcast Paranoia: a horror anthology»). Достижения `CT_WIN_GAME` / `CT_ALL_BOOKS` должны быть заведены и опубликованы в Steamworks именно для него.
- **Поменял `steam_appid.txt` — перезапусти Unity.** Steam API читает файл только при первом `Init` в процессе, а дальше берёт ID из переменных окружения `SteamAppId`/`SteamGameId`, которые сам и выставил; следующие Play в том же редакторе видят старый ID. `SteamAchievements.Bootstrap` в редакторе сверяет файл с тем, что отдал Steam, и пишет предупреждение с этой подсказкой.
- **Под Spacewar имена подменяются сами.** Впиши в `steam_appid.txt` **480** (тестовый Spacewar от Valve), и если Steam отдал этот App ID, `SteamAchievements` при инициализации пишет предупреждение и дальше открывает вместо `CT_WIN_GAME` → `ACH_WIN_ONE_GAME` (Winner), вместо `CT_ALL_BOOKS` → `ACH_TRAVEL_FAR_SINGLE` (Orbiter). Так вся цепочка проверяется без заведения достижений в Steamworks: запущенный Steam + Play → в консоли `Достижение открыто`, в клиенте Библиотека → Spacewar → Достижения. Оверлей к окну редактора обычно не цепляется, плашку увидишь только в standalone-билде с `steam_appid.txt` рядом с exe. Для быстрого теста: `StartingChapter` на `StoryManager` = последняя глава (титры), `_totalOverride` = 1 на `BookCollectedUI` (первая книга закрывает коллекцию); после теста верни 0 и сбрось коллекцию в паузе.
- Отладка: `Tools → Steam → Показать состояние` / `Сбросить достижения` (только в Play Mode при запущенном Steam).

## CI/CD

[.github/workflows/main.yml](.github/workflows/main.yml) — одна сборочная джоба `build-windows` на `game-ci/unity-builder@v4` (джобы `build-macos` и `build-webgl` убраны из пайплайна по решению автора; в git-истории они есть, вернуть можно из коммита до этого), версия Unity берётся из переменной репозитория `vars.UNITY_VERSION`, лицензия и креды — из секретов. **Апгрейдишь проект — обнови и переменную:** после перехода на 6000.6 CI с прежним значением падал на резолве пакетов (модули `physicscore2d`/`tetgen`/`timelinefoundation` и Timeline 6.6 в 6000.3 не существуют). `unityVersion: auto` пробовали — на этом пайплайне не собирает, поэтому переменная, а не автоопределение из `ProjectVersion.txt`. **Windows-билд собирается на `ubuntu-latest`, а не на `windows-latest`:** раннер Windows Server 2025 не поднимает Docker-демон, а game-ci на Windows работает только через Docker (`failed to connect to the docker API`). Standalone у проекта на Mono (IL2CPP только у Android), поэтому Linux-образ `windows-mono` подходит; понадобится IL2CPP под Windows — придётся возвращать Windows-раннер с Docker (например, `windows-2022`). Образ game-ci под новую версию Unity должен существовать на Docker Hub (`unityci/editor`), обычно появляется через несколько дней после релиза. После сборок четвёртая джоба `deploy-itch` качает артефакты и пушит их через `butler` на itch.io по каналам `windows`, `macos`, `html`. Триггеры: `workflow_dispatch` и `repository_dispatch` типа `unity-build-trigger`.
